...
const themeToggler = document.querySelectorAll(".header-toggle-btn");
...
const categories = document.querySelectorAll(".options-category");
const remainingTasks = document.querySelector(".options-remaining");
...
optionsTab.addEventListener("click", e => {
    ...
    if (target.matches(".options-category")) {
        filterList(target);
    } else if (target.matches(".options-clear")) {
        clearCompleted();
    }
});
...
function filterList(selectedCategory) {
    ...
    const activeSelectedCategory = () => {
        categories.forEach(category => {
            category.classList.remove("options-category--active");
            selectedCategory.classList.add("options-category--active");
        })
    };
